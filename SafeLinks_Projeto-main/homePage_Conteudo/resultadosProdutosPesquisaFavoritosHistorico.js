...
resultadosProdutosSearchInput.addEventListener("keydown", async function(event) {
    if (event.key === 'Enter') {
        event.preventDefault();
        await processarEntradaUsuario(resultadosProdutosSearchInput.value);
    }
});

resultadosProdutosSearchButton.addEventListener('click', async function() {
    await processarEntradaUsuario(resultadosProdutosSearchInput.value);
});
